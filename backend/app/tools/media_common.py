...
def transcribe_audio_file(path: Path) -> str:
    """Transcribe a single already-extracted audio file and return the joined
    text. Used when the caller has already chopped a video into per-chunk WAVs
    and wants the transcript inline for that chunk (no whole-file pass)."""
    cfg = settings()
    if cfg.transcribe_azure:
        try:
            return _transcribe_audio_file_azure(path)
        except Exception as exc:  # noqa: BLE001
            log.warning("Azure Whisper failed for %s (%s); falling back to local.",
                        path.name, exc)
    return _transcribe_audio_file_local(path)


def _transcribe_audio_file_azure(path: Path) -> str:
    ...
    resp = _with_retry(_call, what=f"Whisper {path.name}")
    parts: list[str] = []
    for seg in _iter_segments(resp):
        ...
        if text:
            parts.append(text)
    if not parts:
        full = getattr(resp, "text", None)
        if full is None and isinstance(resp, dict):
            full = resp.get("text")
...
def _transcribe_audio_file_local(path: Path) -> str:
    try:
    ...
    parts = [(s.get("text") or "").strip() for s in result.get("segments", [])]
