"""video source tool — whole-file transcribe + per-chunk frame analysis.

For each time chunk of a video this produces a single :class:`ContentBlock`:

* ``text``      — the chunk transcript (sliced from the whole-file transcription),
                  with each kept frame's visual description folded in so visuals
                  are searchable.
...
The video is transcribed once up front (Azure Whisper or local openai-whisper)
and the resulting time-stamped segments are sliced per chunk. Frames are
extracted, deduplicated, captioned and emitted as blocks chunk-by-chunk.

Results are cached under ``processing/transcripts/<file>.transcript.json`` and
reused while the source is unchanged.
"""
...
from app.core.ckm import ContentBlock, Frame
from app.core.config import settings
from app.core.logging import get_logger

from . import media_common
from ._base import OnBlock, _emit, _slug, assets_dir, transcripts_dir

log = get_logger("tools.video")


def extract_video(path: Path, on_block: OnBlock = None) -> list[ContentBlock]:
    cache = transcripts_dir() / f"{path.name}.transcript.json"
    ...
    log.info("transcribing %s (one pass)…", Path(path).name)
    try:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for idx in range(num_chunks):
            ...
            log.info("chunk %d/%d [%.1fs-%.1fs] %.1fs — frames…",
                     idx + 1, num_chunks, chunk_start, chunk_end, chunk_duration)
...
def _chunk_transcript(segments: list[dict], start: float, end: float) -> str:
    """Join segment texts whose midpoint falls inside ``[start, end)``."""
    parts: list[str] = []
    for seg in segments:
